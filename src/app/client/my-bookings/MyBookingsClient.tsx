...
export function MyBookingsClient() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setData(null);
    try {
      const result = await getClientBookingsAction(email.trim());
    ...
    <div className="mt-4 space-y-6">
      ...
      {data && !hasResults && (
        <div className="rounded-xl border border-stone-200 bg-stone-50/50 p-6 text-center text-stone-600">
          ...
          <Link href="/client/packages" className="text-teal-600 hover:underline">
            Browse packages
          </Link>{" "}
          to make a request.
        </div>
            ...
            <div>
              ...
              <div className="mt-4 space-y-3">
                {data.requests.map((lead) => (
                  <Link
                    key={lead.id}
                    href={`/client/booking/${encodeURIComponent(lead.reference ?? lead.id)}?email=${encodeURIComponent(email.trim())}`}
            ...
            <div>
              ...
              <div className="mt-4 space-y-3">
                {data.tours.map(({ tour, package: pkg }) => (
                  <Link
                    key={tour.id}
                    href={`/client/booking/${encodeURIComponent(tour.id)}?email=${encodeURIComponent(email.trim())}`}
                    className="block rounded-xl border border-white/50 bg-white/70 p-4 shadow-lg backdrop-blur-xl transition hover:border-teal-200 hover:shadow-xl"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
